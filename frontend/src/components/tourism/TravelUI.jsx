...
import { mapLink } from '../../data/tourismGuides';
...
export function TravelPhoto({ image, className = 'aspect-[16/9] w-full', eager = false }) {
  ...
  return <figure><SafeImage src={image.url} alt={image.alt} loading={eager?'eager':'lazy'} className={className+' object-cover'} fallbackLabel="Photograph could not load"/><figcaption className="px-4 py-2 text-[10px] leading-4 text-body/65"><a href={image.source} target="_blank" rel="noreferrer" className="underline">{image.alt}</a> · {image.author} · <a href={image.licenseUrl} target="_blank" rel="noreferrer" className="underline">{image.license}</a> · Cropped</figcaption></figure>;
}
...
export function LiveDataNotice({ query, label }) {
  if (query.isError) return <p role="alert" className="mb-5 rounded-lg border border-border bg-surface p-4 text-sm">{label} could not load. The reference guide is still available. <button className="font-semibold text-link underline" onClick={()=>query.refetch()}>Retry</button></p>;
