...
export default function NewProductForm({ initial, onCreated, onCancel }) {
  ...
  return <form className="mb-6 space-y-4 rounded-xl border border-border bg-surface p-5" onSubmit={(event) => {
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="space-y-2 text-sm sm:col-span-2"><label htmlFor="product-photo" className="font-medium">Product photo *</label><input id="product-photo" required={!form.imageUrl} type="file" accept="image/jpeg,image/png,image/webp" className={inputClass} onChange={event => { const file = event.target.files?.[0]; setUploadError(''); if (!file) return; if (file.size > 5 * 1024 * 1024) { setUploadError('Choose an image smaller than 5 MB.'); return; } upload.mutate(file); }} /><p className="text-xs text-muted">JPG, PNG or WebP, up to 5 MB. {upload.isPending ? 'Uploading…' : form.imageUrl ? 'Photo uploaded.' : ''}</p>{(uploadError || upload.isError) && <p role="alert" className="text-red-700">{uploadError || 'The photo could not be uploaded. Try again.'}</p>}{form.imageUrl && <SafeImage src={resolveMediaUrl(form.imageUrl)} alt="Product preview" className="h-40 w-40 rounded-xl border border-border object-cover" />}</div>
