...
import { toPlace, toPhoto } from '../../utils/tourismAdapters';
...
export default function DistrictDetails() {
  ...
  const reference=district&&{knownFor:district.knownFor,description:district.description,source:district.sourceUrl,image:toPhoto(district.imageUrl,district.name,district.imageCredit)};
  const placesQuery=useHeritagePlaces({districtId,pageSize:50});
  const villagesQuery=useVillages();
  const cuisinesQuery=useLocalCuisines({districtId,pageSize:50});
  const places=(placesQuery.data?.items||[]).map(toPlace);
  const villages=(villagesQuery.data||[]).filter(v=>v.districtId===districtId);
  const cuisines=(cuisinesQuery.data?.items||[]).filter(c=>!c.isNationwide);
