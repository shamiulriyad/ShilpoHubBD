...
function Field({
  ...
  if (field.type === 'image-upload') return <div><label htmlFor={common.id} className="mb-1.5 block text-sm font-medium">{field.label}</label><input id={common.id} type="file" accept="image/jpeg,image/png,image/webp" className={inputClass} disabled={upload.isPending} onChange={event => { const file = event.target.files?.[0]; if (file && file.size <= 20 * 1024 * 1024) upload.mutate(file); }} /><p className="mt-1 text-xs text-body/60">JPG, PNG or WebP, up to 20 MB. {upload.isPending ? 'Uploading…' : value ? 'Image ready.' : ''}</p><ErrorNotice error={upload.error} />{value && <SafeImage src={resolveMediaUrl(value)} alt="Uploaded preview" className="mt-2 h-32 w-full rounded-lg object-cover" />}</div>;
