...
import { useCsrOpportunities, useMyCsrProposals, useCsrSponsorshipMutations } from '../../hooks/useCsrSponsorship';
...
export default function SponsorshipMarketplace() {
  const opportunitiesQuery = useCsrOpportunities({ status: 'Open', pageSize: 20 });
  const proposalsQuery = useMyCsrProposals();
  const { submitProposal } = useCsrSponsorshipMutations();
  ...
  const handleSubmit = (id) => {
    submitProposal.mutate(
      { id, payload: { fundingAmount: Number(form.fundingAmount), proposalMessage: form.proposalMessage } },
  ...
  return (
    <div>
      <PageHeader title="Sponsorship Marketplace" description="Sponsor CSR opportunities posted by heritage producers." />
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {opportunities.map((opp) => (
            <div key={opp.id} className="rounded-xl border border-border bg-surface p-4">
              ...
              <p className="mt-2 text-sm text-body/70">{opp.description}</p>
              <p className="mt-2 text-sm font-semibold text-primary">৳ {opp.fundingSecured.toLocaleString()} / ৳ {opp.fundingGoal.toLocaleString()}</p>
      ...
      <AsyncState isLoading={proposalsQuery.isLoading} isError={proposalsQuery.isError} error={proposalsQuery.error}>
        <div className="divide-y divide-border rounded-xl border border-border bg-surface">
          {(proposalsQuery.data?.items || []).map((p) => (
            <div key={p.id} className="flex items-center justify-between p-3 text-sm">
              <span>{p.opportunityTitle}</span>
              <div className="flex items-center gap-3">
                <span className="text-body/60">৳ {p.fundingAmount.toLocaleString()}</span>
                <Badge tone={proposalTone[p.status] || 'neutral'}>{p.status}</Badge>
              </div>
            </div>
          ))}
          {(proposalsQuery.data?.items || []).length === 0 && <p className="p-3 text-sm text-body/60">No proposals submitted yet.</p>}
        </div>
      </AsyncState>
    </div>
  );
