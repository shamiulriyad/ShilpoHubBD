...
public interface IProcurementRepository
{
    ...
    Task<(List<ProcurementRequest> Items, int TotalCount)> GetPagedForProducerAsync(Guid producerId, ProcurementQueryParameters parameters, CancellationToken cancellationToken);
    Task<(List<ProcurementRequest> Items, int TotalCount)> GetPagedForInspectionAsync(bool pendingOnly, ProcurementQueryParameters parameters, CancellationToken cancellationToken);
